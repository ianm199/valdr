...
/// Estimate the geohash step precision needed to cover `range_meters` at
/// given latitude.
...
/// Compute the axis-aligned bounding box for `shape` and store it
/// `shape.bounds`.
/// For `GeoShapeKind::Polygon`, the function also derives the centroid
...
pub fn geohash_calculate_areas_by_shape_wgs84(shape: &mut GeoShape) -> Option<GeoHashRadius> {
 ...
 // Determine whether the current step is fine-grained enough to cover
...
/// The geohash `bits` are shifted left so that the most significant bit of
...
/// When the longitude difference is 0, `asin(sqrt(a))` reduces
/// `asin(sin(|Δlat|/2)) * 2 = |Δlat|` (for latitudes in [-π/2, π/2]), so
...
/// Test whether `point` lies inside the polygon defined by `vertices` using
/// ray-casting algorithm (PNPOLY by W. Randolph Franklin).
/// If the point is inside, returns `Some(distance)` where distance is
