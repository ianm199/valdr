...
//! Translates Valkey's vtable-based
...
/// Enough for an IPv6 address string.
pub const NET_IP_STR_LEN: usize = 46;
/// Must accommodate `hostname:port`.
pub const NET_HOST_PORT_STR_LEN: usize = NET_HOST_STR_LEN + 32;
/// Maximum number of bind-address entries per listener.
...
/// qualifies — `mio::net::TcpStream` in the server, an in-memory `PipeEnd`
...
/// take `&mut self`. All per-connection I/O methods take `&self` because
...
impl Connection {
 ...
 /// true when a write-ready handler is installed.
    pub fn has_write_handler(&self) -> bool {
        self.write_handler.is_some()
    }

 /// true when a read-ready handler is installed.
    pub fn has_read_handler(&self) -> bool {
        self.read_handler.is_some()
    }

 /// true when the last error was EINTR.
 ...
 /// return the connection type discriminant.
    pub fn get_type(&self) -> ConnectionTypeId {
        self.type_id
    }

 /// true when the connection is managed by the TLS backend.
    pub fn is_tls(&self) -> bool {
        self.type_id == ConnectionTypeId::Tls
    }

 /// write a short descriptor (e.g. `fd=5`) into `buf`.
...
/// PORT NOTE: C read `server.listeners[j]` (a process global). Rust makes
/// dependency explicit: callers pass the server's listener slice. This avoids
/// a hidden dependency on `RedisServer` and makes the function testable
/// isolation.
...
// ─── Dispatch wrappers for inline helpers ─────────────────
