//! Thread-safe, two-priority-level FIFO queue.
...
//! alongside two `fifo *` pointers. In Rust the mutex and condvar live in
...
impl<T> MutexQueue<T> {
 ...
 /// Corresponds to `mutexQueueRelease` in C. In Rust, memory cleanup for
 /// inner FIFOs and synchronisation primitives is handled automatically by
 /// [`Drop`]; this method exists solely to enforce the C precondition that
 ...
 // wake any threads still blocking on wait.
