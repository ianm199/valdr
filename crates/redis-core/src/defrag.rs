...
//! * **Deferred data structures** — kvstore, quicklist, rax, zset, stream,
...
//! (e.g. `tikv-jemallocator`). Define a `DefragAllocator` trait
...
/// In C, this struct MUST begin at offset 0 within the `defragKeysCtx`
...
/// - `kvstore_iter_state`: from `static kvstoreIterState state`
/// `defragStageKvstoreHelper`
/// - `prev_cpu_percent`: from `static int prevCpuPercent`
...
/// should this allocation be relocated?
#[inline(always)]
fn allocator_should_defrag() -> bool {
    false
}

/// fresh allocation outside the thread cache.
...
/// allocator-tracked allocation size.
fn zmalloc_size() -> usize {
    0
}

/// fragmentation ratio and bytes.
...
/// bgsave/bgrewrite/etc. in progress.
...
/// PORT NOTE: In C, `sds` is a header + data block; defrag adjusts
...
fn defrag_stage_kvstore_helper(
 ...
 // defrag the main hashtable struct / bucket arrays before iterating.
 ...
 // scan each non-empty kvstore slot
    let mut iterations: u32 = 0;
    loop {
        iterations += 1;
        if iterations > 16 {
...
fn end_defrag_cycle(defrag_ctx: &mut DefragContext, normal_termination: bool) {
 ...
 // check if another cycle should begin immediately.
...
fn compute_defrag_cycle_us(
 ...
 // P) — duty time to achieve target CPU%
...
/// Called frequently while defrag is running; returns the ms delay until
...
/// In C this reads from and writes to `server.*` directly. In Rust
...
pub fn monitor_active_defrag(
 ...
 // adjusts server.active_defrag_cpu_percent.
