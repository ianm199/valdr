...
//! channel that `PubSubRegistry` uses) plus a `BlockSpec` describing how
...
//! the LIST family's push hook drains FIFO entries via [`take_waiter`];
...
/// millisecond deadline. A `seconds` of `0.0` means block forever and maps
/// `i64::MAX`.
