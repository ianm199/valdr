...
//! In Rust they live in [`TrackingState`], intended to become a field
//! `RedisServer`.
...
/// Holds the set of keys modified in this event-loop cycle and the set
/// client IDs subscribed to notifications for this prefix.
#[derive(Debug)]
pub struct BcastState {
 /// Keys modified in the current event-loop cycle.
 /// Maps key bytes → the client ID that last modified the key, used
 /// implement the NOLOOP option (skip sending the notification back to
...
/// `TrackingTable`, `PrefixTable`, `TrackingTableTotalItems`,
...
/// `trackingGetTotalKeys`, `trackingGetTotalPrefixes` in/.
...
/// Decouples `tracking.rs` from the concrete `Client` type so that
...
/// Returns `true` if `s1` and `s2` share a common prefix
/// `min(s1.len, s2.len` bytes.
/// `memcmp(s1, s2, min_length) == 0`.
...
/// If `c` is in BCAST mode, iterates all prefixes it is subscribed,
...
/// a descriptive message if two prefixes overlap (i.e., one is a prefix
...
/// tracking. Sets tracking flags on the client, in BCAST mode, subscribes
/// to all supplied prefixes (or the empty prefix if none are given).
/// `tracking_clients_was_zero` returns `true` if `tracking_clients` needs
...
/// In optin/optout mode the function checks the `tracking_caching` flag
...
/// Handles:
/// * Redirection — if `c` has a `client_tracking_redirection`, look up
...
/// by ID without holding a mutable reference to the full server client list
/// the same time.
...
/// Called after the command response has been written, before returning
...
/// `TrackingState` struct. Also the random-walk selection
