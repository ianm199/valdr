...
//! (`ChildInfoSender` for the child side, `ChildInfoChannel` for
//! parent side) so that ownership is explicit and state is not hidden
...
/// `receive_child_info` calls, mirroring the C static-local buffer
/// `readChildInfo`.
...
/// Rust must construct a non-blocking pipe without `unsafe`;
...
/// In C, only closes if at least one fd is not −1; Rust simply drops
...
/// - `now_us` — current monotonic timestamp (µs); injected
/// avoid calling `getMonotonicUs` inside this fn.
/// PORT NOTE: The C function calls `getMonotonicUs` twice (before
...
pub fn send_child_info_generic(
 ...
 // CoW throttle + measurement block
 // Measure CoW if:
...
/// PORT NOTE: The C function stores a `static child_info_data buffer`
...
/// Drain all pending child-info packets from the pipe and apply each
/// server statistics.
