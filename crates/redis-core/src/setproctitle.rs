...
//! (NetBSD, FreeBSD, OpenBSD, DragonFly) ship a native `setproctitle(3)`
//! do not need this implementation.
//! ## Technique
//! The C implementation overwrites `argv[0]` in-place. The kernel exposes
//! null-terminated string at `argv[0]` as the process command line (visible
...
//! This approach requires manipulating raw pointers into process memory
...
struct SptState {
 ...
 // the raw C `argv` array passed to `main`. There is no safe Rust API
 // obtain them.
}
...
/// Deep-copies environment variables from the original `environ` block so
/// strings no longer reside in the argv-adjacent memory region.
/// After this call, the process `environ` pointer is redirected
...
/// heap-allocated strings; the raw `argv` pointers are never exposed by
...
/// delegate to the OS implementation. On Linux and macOS it overwrites
