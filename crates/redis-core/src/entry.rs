...
//! optional expiry.
//! - **Type 3** (`SDS_TYPE_8+` field, value pointer): field + pointer
//! separately allocated sds, optional expiry.
//! - **Type 4** (`SDS_TYPE_8+` field, stringRef pointer): field + pointer
...
// RedisString + RedisError; both crates are already listed
...
pub enum EntryValue {
 ...
 /// Covers embedded values (Types 1/2, where C co-locates field + value
 ...
 /// struct but NOT the buffer it points. Freeing a Type 4 entry frees
...
/// Maps to the C `entry` opaque type (`typedef struct _entry entry`
/// ``). In C the entry pointer IS the field SDS data pointer —
...
impl Entry {
 ...
 /// Returns the value as a byte slice.
 /// PORT NOTE: C writes the length via an out-pointer `size_t *len`
 ...
 /// separately allocated sds for the value but is still "owned" by
 /// entry. Type 4 (`stringRef`) is the only case the C code treats as
 /// "not embedded."
 /// In Rust, `Owned(...)` covers all owned cases (C Types 1–3)
 ...
 /// buffer. The entry stores the `Arc<[u8]>` but does not own
 ...
 /// and stores a pointer to it inside the entry allocation. In Rust
 ...
 /// expiry changed. The Rust version is equivalent: assigning
 ...
 /// PORT NOTE: The C implementation calls `zmalloc_usable_size` on
 ...
 /// `madvise(MADV_DONTNEED)` on the value's backing pages — intended
...
/// In C, `entryFree` manually frees the value pointer (if any) and
